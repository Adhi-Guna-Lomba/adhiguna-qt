...
int main(int argc, char *argv[])
{
             ...
             switch (c)
               ...
               case '?':
                 if (optopt == 'c')
                   ...
                   fprintf (stderr, "Unknown option `-%c'.\n\nUsage: adhigunasdk -p [PROJECT DIRECTORY]\n\nGeneral Options :\n-v\t--version\t\toutput version information and exit\n-d\t--dev\t\t\tWeb Inspector Enable\n-r\t--dev-remote [port]\tRemote debugging\n\nWindow options :\n--transparent\tTransparent Mode\n--noFrame\tFrame Disable\n", optopt);
                 else
                   fprintf (stderr,
    ...
    if(version){
        qDebug() << "adhiguna Version : "<< w.sdkVersion();
        exit(0);
    }
    ...
    if(!opt.isEmpty()){
        ...
        if (result)
        {
            ...
            QMessageBox::warning(0,"Warning","Anda tidak memilih Berkas Apapun\n");
            exit(0);
        }
    }
